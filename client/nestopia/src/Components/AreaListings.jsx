...
const AreaListings = () => {
    const major_area = useParams()
    const [data, setData] = useState([]);

useEffect(() => {
    const fetchListing = async () => {
  axios.get(`http://127.0.0.1:5050/all_listings?major_area=${major_area.major_area}`)
    .then(response => {
               ...
               <div key={element.id} className="crn-property-thumb-wrapper mb-60">
                  ...
                  <div className="crn-property-content">
                     ...
                     <Link to={`/single_listing/${element.id}`} className="crn-property-title">{element.name}</Link>
                        ...
                        <h3 style={{textDecoration: 'none'}}><i className="fa-location-dot"></i>{element.location}</h3>
